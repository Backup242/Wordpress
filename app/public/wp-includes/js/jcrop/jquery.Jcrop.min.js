  ...
  ($.Jcrop = function (obj, opt) {
    ...
    function startDragMode(mode, pos, touch) {
      if (
        ...
        Tracker.setCursor("move" === mode ? mode : mode + "-resize"),
        "move" === mode)
      )
        ...
        var animator = (function () {
          return function () {
            (pcent += (100 - pcent) / velocity),
              ...
              100 > pcent
                ? (setSelectRaw(animat), queueAnimator())
                : (Selection.done(),
    ...
    function interfaceUpdate(alt) {
      options.allowResize
        ...
        bgopacity != options.bgOpacity &&
          ((bgopacity = options.bgOpacity),
          options.shade ? Shade.refresh() : Selection.setBgOpacity(bgopacity)),
    ...
    if ("IMG" == obj.tagName) {
      if (0 != $origimg[0].width && 0 != $origimg[0].height)
        $origimg.width($origimg[0].width), $origimg.height($origimg[0].height);
      else {
    ...
    var Touch = (function () {
        ...
        function detectSupport() {
          return options.touchSupport === !0 || options.touchSupport === !1
      ...
      Coords = (function () {
        ...
        function getFixed() {
          ...
          return (
            ...
            xx > x1
              ? (min_x > xx - x1
                  ? (xx = x1 + min_x)
                  : xx - x1 > max_x && (xx = x1 + max_x),
                (yy =
                  yy > y1 ? y1 + (xx - x1) / aspect : y1 - (xx - x1) / aspect))
              : x1 > xx &&
                (min_x > x1 - xx
                  ? (xx = x1 - min_x)
                  : x1 - xx > max_x && (xx = x1 - max_x),
                (yy =
                  yy > y1 ? y1 + (x1 - xx) / aspect : y1 - (x1 - xx) / aspect)),
            0 > xx
              ? ((x1 -= xx), (xx = 0))
              : xx > boundx && ((x1 -= xx - boundx), (xx = boundx)),
            0 > yy
              ? ((y1 -= yy), (yy = 0))
              : yy > boundy && ((y1 -= yy - boundy), (yy = boundy)),
        ...
        function getRect() {
          ...
          return (
            ...
            0 > y2 && ((y1 -= y2), (y2 -= y2)),
            x2 > boundx &&
              ((delta = x2 - boundx), (x1 -= delta), (x2 -= delta)),
            y2 > boundy &&
              ((delta = y2 - boundy), (y1 -= delta), (y2 -= delta)),
            x1 > boundx &&
              ((delta = x1 - boundy), (y2 -= delta), (y1 -= delta)),
            y1 > boundy &&
              ((delta = y1 - boundy), (y2 -= delta), (y1 -= delta)),
      ...
      Shade = (function () {
        ...
        function updateShade(c) {
          shades.top.css({ left: px(c.x), width: px(c.w), height: px(c.y) }),
            shades.bottom.css({
              top: px(c.y2),
              left: px(c.x),
              width: px(c.w),
        ...
        function disableShade() {
          enabled &&
            ...
            Selection.isAwake()
              ...
              : (Selection.setBgOpacity(1, 1, 1), Selection.disableHandles()),
      ...
      Selection = (function () {
        ...
        function moveto(x, y) {
          options.shade || $img2.css({ top: px(-y), left: px(-x) }),
      ...
      Tracker = (function () {
        ...
        function activateHandlers(move, done, touch) {
          return (
            (btndown = !0), (onMove = move), (onDone = done), toFront(touch), !1
          );
        }
      ...
      KeyManager = (function () {
        ...
        function doNudge(e, x, y) {
          options.allowMove &&
            (Coords.moveOffset([x, y]), Selection.updateVisible(!0)),
            e.preventDefault(),
            e.stopPropagation();
        }
        function parseKey(e) {
